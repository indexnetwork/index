---
name: bump-agentvillage-pointer
...
in `create-worktree`: it needs no dependency install, but the worktree must have
...
`create-worktree` → "Commit signing (GPG fails without a TTY)".
...
- `create-worktree` — worktree conventions + GPG signing escape.
- `docs/guides/agentvillage-submodule.md` + CLAUDE.md → Subtrees → agentvillage submodule.
- `inspect-edge-city-railway` — verifying sidecar state after rollout.
