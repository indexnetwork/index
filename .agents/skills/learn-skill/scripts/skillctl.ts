 ...
 * project-local target (default: .agents/skills relative to cwd).
 *
 * Commands:
...
function localSkillFiles(): string[] {
  ...
  for (const entry of entries) {
    ...
    if (!entry.isDirectory() || entry.name === config.modularize.sharedDir) continue;
    files.push(join(path, "SKILL.md"));
  }
  return files.sort((a, b) => repoRelative(a).localeCompare(repoRelative(b)));
}

function validateAgentMetadata(skillFile: string, name: string | null, errors: string[]): void {
  const metadataFile = join(dirname(skillFile), "agents", "openai.yaml");
  ...
  try {
    metadata = parseYaml(readFileSync(metadataFile, "utf8"));
  } catch (error) {
    const message = error instanceof Error ? error.message.split("\n")[0] : String(error);
    errors.push(`agents/openai.yaml has invalid YAML: ${message}`);
    return;
  }
  ...
  const agentInterface = (metadata as Record<string, unknown>).interface;
  if (agentInterface === null || typeof agentInterface !== "object" || Array.isArray(agentInterface)) {
  ...
  for (const key of ["display_name", "short_description", "default_prompt"] as const) {
    const value = (agentInterface as Record<string, unknown>)[key];
    if (typeof value !== "string" || value.trim() !== value || value.length === 0) {
      errors.push(`agents/openai.yaml interface.${key} must be a nonempty trimmed string`);
    }
  }
  const defaultPrompt = (agentInterface as Record<string, unknown>).default_prompt;
  if (name && typeof defaultPrompt === "string" && !defaultPrompt.includes(`$${name}`)) {
    errors.push(`agents/openai.yaml interface.default_prompt must reference $${name}`);
  }
}

function validateReferences(skillFile: string, errors: string[]): void {
  const text = readFileSync(skillFile, "utf8");
...
function validateSkill(skillFile: string, duplicateNames: ReadonlySet<string>): SkillValidation {
  ...
  if (!existsSync(skillFile) || !statSync(skillFile).isFile()) {
    ...
    return { path: repoRelative(skillFile), name: null, errors };
  }
  ...
  if (isDirectFileSkill) {
    errors.push("direct-file skills are not allowed; move this skill to .agents/skills/<name>/SKILL.md");
  ...
  validateAgentMetadata(skillFile, name, errors);
  validateReferences(skillFile, errors);
  return { path: repoRelative(skillFile), name, errors };
}
...
function ensureAgentMetadata(skillDir: string, name: string): boolean {
  ...
  writeFileSync(
    metadataFile,
    `interface:\n  display_name: ${JSON.stringify(title)}\n  short_description: ${JSON.stringify(`Index project workflow: ${title}`)}\n  default_prompt: ${JSON.stringify(`Use $${name} to follow the repository-specific ${title} workflow.`)}\n`,
  );
  return true;
}
...
switch (cmd) {
  ...
  case "migrate": {
    ...
    const addedMetadata = ensureAgentMetadata(dest, arg);
    console.log(`migrated ${prot.dir} -> ${dest}`);
    if (addedMetadata) console.log(`created ${join(dest, "agents", "openai.yaml")}`);
  ...
  case "validate": {
    ...
    if (!asJson && arg === "all") console.log(`Validated ${report.skills.length} Agent Skills.`);
    break;
  }
