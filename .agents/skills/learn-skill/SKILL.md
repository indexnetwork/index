...
```bash
bun .agents/skills/learn-skill/scripts/skillctl.ts detect
```

## Workflow

Run all commands from the project root (so `cwd` is the project whose `.agents/skills` should receive the skill). Use `bun` — it is the repo runtime.
...
```bash
bun .agents/skills/learn-skill/scripts/skillctl.ts similar <keywords...>
```
...
```bash
bun .agents/skills/learn-skill/scripts/skillctl.ts plan <skill-name> <keywords...>
```

### 2. Resolve the write action

```bash
bun .agents/skills/learn-skill/scripts/skillctl.ts resolve <skill-name>
```
...
```bash
bun .agents/skills/learn-skill/scripts/skillctl.ts list
bun .agents/skills/learn-skill/scripts/skillctl.ts locate <skill-name>
```

### 3a. CREATE

Write `<target>/<skill-name>/SKILL.md` with valid frontmatter (`name`, `description`)
and a concise body. Also write `<target>/<skill-name>/agents/openai.yaml` with nonempty
`interface.display_name`, `interface.short_description`, and `interface.default_prompt`
strings; the default prompt must reference `$<skill-name>`. The `description` must state
**what it does and when to use it** — this is what triggers loading later. Add `scripts/`
or `references/` only if needed.
...
the relevant section, tighten the description if the trigger conditions widened, and
add or refresh `agents/openai.yaml` when needed. Do not rewrite wholesale.

### 3c. MIGRATE (protected → local), then update

```bash
bun .agents/skills/learn-skill/scripts/skillctl.ts migrate <skill-name>
```

This copies the protected skill directory into `target`, creates compatible
`agents/openai.yaml` metadata when the source does not have it, and leaves the original
...
```bash
bun .agents/skills/learn-skill/scripts/skillctl.ts audit <skill-name>
```
...
```bash
bun .agents/skills/learn-skill/scripts/skillctl.ts validate <target>/<skill-name>
bun run skills:validate
```

Fix every reported error (valid YAML mapping, action-object name rules, parent-directory
match, uniqueness, a trimmed nonempty string description ≤1024 chars, valid
`agents/openai.yaml` metadata, and resolvable local Markdown references). A skill with a
missing description will not load. Use `validate all --json` when a deterministic
