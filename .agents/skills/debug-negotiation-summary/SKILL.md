---
name: debug-negotiation-summary
...
Get control-plane creds from its Railway vars (`CONTROL_PLANE_API_KEY`, domain `control-plane-production-b752.up.railway.app`) — see `inspect-edge-city-railway` for the snake_case IDs. Then for the tenant (look up by email):
...
See also: `inspect-edge-city-railway` (sidecar IDs + the snake_case gotcha), `audit-digests` (digest card quality).
