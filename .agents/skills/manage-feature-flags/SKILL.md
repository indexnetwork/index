...
Six related flags, all with accessors in `packages/protocol/src/opportunities/discriminator/discriminator.env.ts` and documented commented-off in `.env.example` section 13: `POOL_QUESTIONS_MINING`, `POOL_QUESTIONS_MODE`, `POOL_QUESTIONS_PUSH`, `POOL_QUESTIONS_RANKING`, `POOL_QUESTIONS_STAMP_NEWBORN`, `POOL_QUESTIONS_VISIT_TRIGGER`. This list is the *names* only — do not record live values here, they rot. Check current values on all applicable surfaces with the Quick audit above (`.env.development`, conditional `.env.test`, and `railway_list_variables`) before reasoning about pool-question behavior.
