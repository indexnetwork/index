...
- Do not merge without explicit user confirmation in the current session. The only exception is internal mode (below): a wave handoff may pre-authorize merges **into an integration branch only** — never into `dev` or `main`. Those internal merges are executed by the **root session** itself; children never merge.
...
- **Merge execution stays with root.** The root session performs the squash merge
  itself from its own integration-branch worktree, after verifying that worktree's
  path, branch, and clean status and after its own local gates pass — never on a
  child's claims alone. Children never merge. Standing wave authorization covers
  these internal merges only; promotion of the integration branch itself into `dev`
  always needs fresh explicit user confirmation.
...
- **Version collisions** between parallel internal PRs use the base-version floor
  rule from step 3b; the root session performs the deliberate SemVer/lockfile
  reconciliation.
