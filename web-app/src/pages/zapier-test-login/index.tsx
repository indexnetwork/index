...
const ZapierTestLoginPage: NextPageWithLayout = () => {
	...
	async function create(e: any) {
		e.preventDefault();
		const testSession = await apiService.zapierTestLogin(e.target[0].value, e.target[1].value);
		if (!testSession.success) return;
		const sessionStr = Buffer.from(testSession.sessionData, "base64").toString("utf8");
		...
		localStorage.setItem(`pkp_${index.pkpPublicKey}`, JSON.stringify({
			isCreator: true,
			...
			<form onSubmit={create}>
				<Col >
					...
					<Flex flexDirection="column" className={"mt-10"}>
						<Text theme={"primary"} size="md">Email:</Text>
						<Input
							autoFocus={true}
							className="mt-3"
							inputSize={"lg"}
					...
					<Flex flexDirection="column" className={"mt-5"}>
						<Text theme={"primary"} size="md">Password:</Text>
						<Input
							type={"password"}
							autoFocus={true}
							className="mt-3"
