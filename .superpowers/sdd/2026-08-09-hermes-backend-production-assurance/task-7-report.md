...
- All three jobs retain the reviewed official `postgres:16@sha256:95206741a5b214807675e14165369d05b93a9cf692223b616d07cca227e74b0b` multi-architecture image. They share PGDG package version `0.8.6-1.pgdg13+1`, extension version `0.8.6`, SHA-256 `9aea9c1617bc99991d3730cfbf5878a0e9dc377e0d3d5ca2e41488a2309319bc`, the virtual JIT ABI `19`, and the historical archive object's S3 version ID `x3lsgKtr53BtiGMRJqIlPZr52kLw0jvS`. Each job downloads that exact versioned object on the runner, verifies its hardcoded SHA-256 before copying it, proves the base already has installed `postgresql-16` and `libc6`, robustly parses `postgresql-16`'s comma-separated `Provides` and requires exactly one compatible `postgresql-16-jit-llvm (= 19)` entry, validates the package's exact `Depends` and `Breaks: postgresql-16-jit-llvm (<< 19)` metadata, installs only with `dpkg`, and asserts the installed package and live extension versions. No apt index or unversioned package resolution remains.
...
## Minor final-review follow-up

The sole remaining Minor from reviewer artifact `6ef887bd` is resolved. The operational emergency command block now disables history on a best-effort basis before enabling `set -euo pipefail`. Because the history command retains `2>/dev/null || true` and precedes errexit, an unsupported history setting does not abort the procedure; every subsequent failed command, `jq` check/extraction, canonical-integer regex/bound check, or receipt validation does abort before confirmation or terminal manual cleanup can appear successful.

The release contract asserts the exact adjacency of best-effort history disable, fail-closed shell enablement, and `umask`, and asserts that fail-closed semantics precede dry-run plan generation, plan validation, and the separately authorized confirmation step. TDD RED was 14 passed / 1 failed / 216 expectations before the runbook change; focused GREEN was 15 passed / 0 failed / 222 expectations. The relevant provider-free Hermes release matrix passed 77 tests / 0 failures / 576 expectations. The extracted operational Bash block passed `bash -n`, the assurance workflow passed Bun YAML parsing, and `git diff --check` passed. No database, Docker, service container, `DATABASE_URL`, `TEST_DATABASE_SAFE`, provider credential, production action, or push was used.

## CI run 31530480391 JIT virtual-package correction

GitHub Actions run `31530480391` failed in jobs `93909030423` and `93909030452` after the pgvector package hash passed and before package installation. OCI inspection of the pinned Linux/amd64 child `sha256:670391653713782e51974845b217c56fed4dd8729142299c43c919a8d3e15e00` established that installed `postgresql-16` version `16.14-1.pgdg13+1` declares `Provides: postgresql-16-jit-llvm (= 19), postgresql-contrib-16`. Reconstructing its dpkg status database also established that `dpkg-query --show postgresql-16-jit-llvm` exits zero while reporting the virtual name as not installed. The workflow's negated `dpkg-query` command was therefore the exact first-preflight failure, not evidence of an incompatible installed package.

The three jobs now fail closed on the real compatibility boundary: installed `postgresql-16` must contain exactly one comma-delimited provider entry for virtual JIT ABI `19`, and the already hash-verified pgvector deb must declare exactly `Depends: postgresql-16, libc6 (>= 2.38)` and `Breaks: postgresql-16-jit-llvm (<< 19)` before `dpkg --install`. The invalid virtual-package absence query is forbidden by release contracts, no apt resolver was introduced, and the fixed ABI is shared at workflow scope.

Provider-free release/readiness contracts, package byte/hash/control metadata checks, workflow YAML plus every shell run-block parse, frozen install, and diff checks pass locally without Docker, a database, provider credentials, `DATABASE_URL`, or `TEST_DATABASE_SAFE`. A rerun of the exact committed revision in the service-backed GitHub Actions gate is pending; package installation, extension creation, migrations, and guarded database fixtures remain CI-owned evidence and are not claimed from local validation.

## CI run 31532593465 telemetry aggregate correction

GitHub Actions run `31532593465`, job `93915976347`, passed the pinned pgvector installation and migration preflight, then failed in `src/lib/drizzle/tests/hermes-runtime-telemetry.database.isolated.ts` while `HermesRuntimeTelemetryDatabaseAdapter.countActiveCredentialExpiryHealth` executed its aggregate. Direct `PgDialect` rendering established the root cause: all three raw SQL parameters were JavaScript `Date` objects, which postgres.js rejects rather than serializing.

The registered fresh-process adapter test now captures and renders the production SQL and asserts zero `Date` parameters, the exact ordered ISO strings, and explicit `::timestamptz` casts at all three expiry comparisons. It retains the one-query, two-count aggregate, active-only predicate, inclusive/exclusive expiry boundaries, and no-row/privacy assertions. The production adapter now binds `input.now.toISOString()` and `input.nearExpiryCutoff.toISOString()` with those explicit casts; count semantics and error propagation are unchanged.

TDD RED reproduced the defect provider-free: the focused fresh-process target failed with the three rendered `Date` parameters. Focused GREEN passed with 1 test and 9 expectations. The provider-free Task 7 matrix passed 128 tests / 860 expectations; the fresh-process Sentry target passed 2 tests / 6 expectations; and the Mac saga passed 26 tests / 222 expectations. Frozen install, API/protocol build, API and CLI-spec typechecks, API lint (zero errors; 46 pre-existing warnings), focused lint, and `git diff --check` passed. A service-backed rerun of the corrected committed revision is pending; no local database, Docker, `DATABASE_URL`, `TEST_DATABASE_SAFE`, provider credential, push, or production action was used.
