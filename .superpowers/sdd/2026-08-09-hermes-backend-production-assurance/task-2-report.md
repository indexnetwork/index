...
## Scalar PID binding follow-up (run 31507172194)

Exact-head CI reached the deterministic pickup row-lock fixture but postgres.js rejected the waiter query before PostgreSQL execution: interpolating `[247]` into `${knownBlockers}::int[]` supplied scalar `247` to the array serializer. The fixture now builds one boolean term per PID, binds each PID as its own parameter with an explicit `::int` cast, and compares it with `ANY(pg_blocking_pids(activity.pid))`. The terms are composed only with Drizzle `sql.join`; no value enters SQL text, and an empty evidence list fails explicitly.

A provider-free PostgreSQL-dialect contract was observed RED when the helper module was absent, then GREEN with 3 tests / 7 expectations. It proves the exact rendered predicate, scalar-only parameters, explicit casts, empty-list refusal, and parameterization of unexpected input. The focused SQL and isolated-inventory contracts passed with 17 tests / 73 expectations. Targeted and package lint, `tsc --noEmit`, CLI-spec typecheck, the API/protocol build, Bash syntax, workflow YAML parsing, and `git diff --check` pass; package lint reports only its 46 pre-existing warnings. The `pg_stat_activity` active-lock evidence, blocking-graph synchronization, and cadence-only timer remain unchanged; `TEST_DATABASE_SAFE` was not set locally. Dedicated PostgreSQL CI must rerun the authority suite.
