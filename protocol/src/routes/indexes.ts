...
router.post('/:id/members/bulk-import',
  authenticatePrivy,
    ...
    try {
      ...
      for (const row of results) {
        try {
          ...
          if (!email) {
            importErrors.push({ row, error: 'Missing email' });
            continue;
          }

          // Prepare socials object if any social fields are provided
          const socials: any = {};
          if (row.twitter?.trim()) {
            socials.x = row.twitter.trim();
          }
          if (row.website?.trim()) {
            socials.websites = [row.website.trim()];
          }

          // Find or create user by email with all profile fields
          // This will also update empty fields for existing users
          ...
          if (!user) {
            importErrors.push({ row, error: 'Failed to create/find user' });
            continue;
          }
          ...
          const existingMember = await db.select({ userId: indexMembers.userId })
            .from(indexMembers)
            .where(and(eq(indexMembers.indexId, id), eq(indexMembers.userId, user.id)))
            .limit(1);

          if (existingMember.length > 0) {
            importErrors.push({ row, error: 'Already a member' });
            continue;
          }

          // Collect metadata from extra columns
          const metadata: Record<string, string | string[]> = {};
          for (const [key, value] of Object.entries(row)) {
            if (!['email', ...profileFields].includes(key) && value) {
          ...
          importErrors.push({ row, error: String(error) });
        }
      }

      return res.json({
        message: `Successfully imported ${addedMembers.length} member(s)`,
