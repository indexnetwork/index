...
import { intents, intentIndexes, intentStakes } from '../lib/schema';
...
export class IntentService {
  ...
  static async createIntent(options: CreateIntentOptions): Promise<CreatedIntent> {
    ...
    try {
      ...
      console.log(`✅ Created inference stake for intent ${createdIntent.id}: ${inferenceType} (${confidence})`);
    } catch (error) {
      console.error(`Failed to create inference stake for intent ${createdIntent.id}:`, error);
