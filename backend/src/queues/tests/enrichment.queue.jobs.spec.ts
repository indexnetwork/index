...
import { EnrichmentQueue, QUEUE_NAME } from '../enrichment.queue';
...
describe('EnrichmentQueue', () => {
  ...
  describe('processJob', () => {
    it('ensure_profile_hyde invokes profile-write handler with userId', async () => {
      ...
      const queue = new EnrichmentQueue({ invokeProfileWrite });
    ...
    it('enrich.user invokes enrich-user handler with userId', async () => {
      ...
      const queue = new EnrichmentQueue({ invokeEnrichUser });
