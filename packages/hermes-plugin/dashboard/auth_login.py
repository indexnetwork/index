...
def _callback_html(title: str, message: str, ok: bool = False) -> str:
    """Landing-styled response matching the Mac app and CLI login callbacks."""
    check = (
        '<div class="check"><svg viewBox="0 0 24 24" width="26" height="26" fill="none" '
        'stroke="#0b1612" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">'
        '<path d="M20 6L9 17l-5-5"/></svg></div>'
        if ok
        else ""
    )
    return f"""<!doctype html><html><head><meta charset="utf-8"><title>{title} · index</title>
