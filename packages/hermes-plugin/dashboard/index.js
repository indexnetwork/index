...
(function () {
  ...
  function AgentChat(props) {
    ...
    const draftState = useState(function () { return savedDraft().message || ""; });
    const draft = draftState[0];
    const setDraft = draftState[1];
    const selectionsState = useState(function () { return savedDraft().answers || {}; });
    ...
    useEffect(function () {
      try { sessionStorage.setItem(storageKey, JSON.stringify({ message: draft, answers: selections })); }
      catch (_) { /* The draft remains in memory when storage is unavailable. */ }
    ...
    function read() {
      ...
      return fetchPluginJSON(API + "/agent/conversation?intentId=" + encodeURIComponent(intentId))
        .then(function (payload) {
          if (!aliveRef.current || sequence !== readSequence.current || !payload || payload.success === false) return;
    ...
    useEffect(function () {
      return subscribeUserEvents(function (data) {
        const forIntent = data.type === "message"
          ...
          : ["question.pending", "agent.status", "intent.lifecycle"].includes(data.type) && data.data && data.data.intentId;
    ...
    function send() {
      ...
      fetchPluginJSON(API + "/agent/message", {
        ...
        if (!payload || payload.success === false) throw new Error(payload && payload.error || "Message was not accepted.");
        setDraft("");
        return read();
      }).catch(function (failure) {
        setError(failure.message || "Could not save your message. Your draft is kept.");
      }).finally(function () { if (aliveRef.current) setSending(false); });
    }

    function sendAnswers() {
      if (!questions.length || chosen.length !== questions.length || sending || !canSend) return;
      ...
      fetchPluginJSON(API + "/agent/answers", {
        ...
        if (!payload || payload.success === false) throw new Error(payload && payload.error || "Answers were not accepted.");
        setSelections({});
        setWriting({});
        return read();
      }).catch(function (failure) {
    ...
    return React.createElement("div", { ref: rootRef, className: "index-dashboard__agent-chat" },
      React.createElement("p", { className: "index-dashboard__card-description" },
        agent ? "Your inbox for this signal · " + agent.status : "Loading your agent conversation…",
      ),
      error ? React.createElement("p", { role: "alert", className: "index-dashboard__error" }, error) : null,
    ...
    return React.createElement("div", { className: "index-dashboard__detail" },
      ...
      React.createElement("div", { className: "index-dashboard__detail-cols" },
        React.createElement(Panel, { title: "signal" },
          signalHead,
          React.createElement(AgentChat, { key: intent.id, intentId: intent.id, focusQuestion: props.focusQuestion, onOpenUser: props.onOpenUser }),
        ),
        ...
        React.createElement("div", { className: "index-dashboard__agent-rows" },
          [optionRow(
            ...
            return optionRow(
              ...
              agent.name,
              here
                ? "Set INDEX_EXECUTOR_ID=" + (agent.id || "the registered UUID") + " in this device's Hermes environment, then restart its gateway."
                : agent.id
