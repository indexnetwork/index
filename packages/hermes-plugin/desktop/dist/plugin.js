...
export default {
  ...
  register: function (ctx) {
    ...
    // hermes://index-network deep link -> open the plugin tab. Warm links only:
    // a cold-start link is flushed to the app's built-in listener before runtime
    // plugins load, so it opens the app but cannot navigate here.
    const offDeepLink = window.hermesDesktop && window.hermesDesktop.onDeepLink
      ? window.hermesDesktop.onDeepLink(function (payload) {
          if (payload && payload.kind === 'index-network') host.navigate('/index-network')
        })
      : null
    ctx.onDispose(function () { if (offDeepLink) offDeepLink() })
