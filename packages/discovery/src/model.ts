...
/** Back off across HTTP retries and model fallbacks; Retry-After is a minimum wait. */
function retryDelay(response: Response | undefined, failureCount: number): number {
  const backoffMs = 1_000 * 2 ** failureCount;
  const retryAfter = response?.headers.get('retry-after');
  if (!retryAfter) return backoffMs;
  const seconds = Number(retryAfter);
  const retryAfterMs = Number.isFinite(seconds) ? seconds * 1_000 : Date.parse(retryAfter) - Date.now();
  return Number.isFinite(retryAfterMs) ? Math.max(backoffMs, retryAfterMs) : backoffMs;
}
    ...
    for (const model of models) {
      ...
      try {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
          if (delayMs > 0) await sleep(delayMs, undefined, { signal: options.signal });
          options.signal?.throwIfAborted();
          ...
          try {
            ...
            options.signal?.throwIfAborted();
            delayMs = retryDelay(response, failureCount++);
