...
- Principal-agent `wake()` and `brief()` opportunities include complete, ordered `turns` with `speaker: "our_agent" | "counterparty_agent"`, `action`, and `message`, plus `turnCount`, `maxTurns`, and `remainingTurns`. An earlier counterpart answer remains visible after later turns; our negotiation agent's claims do not establish principal facts or permission.
- Standalone `negotiate()` callers supply oldest-first `conversation` and `turns`; `opportunity` carries the turn budget without repeating the history. Both agents receive `remainingTurns` as the total A2A turns left for both seats. `AgentRunner` derives it as `Math.max(0, maxTurns - turnCount)` from existing host reads. Turn pressure never overrides evidence or approval requirements.
- Caller change: `Opportunity.turns` is now a required structured array; `terms` is removed. Supply the three numeric budget fields in both agents' opportunity context. No host API, stored data, or schema changes are needed.
