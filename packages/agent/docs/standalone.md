...
- Bound the whole run using `maxSteps`: 8 for `wake`, 1 for `brief`, and 3 for `negotiate`. One step is a response followed by all its requested tool calls. Stop early on no calls, and keep outputs already collected at the cap.
