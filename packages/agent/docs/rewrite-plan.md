...
- The runner supplies the negotiator with this intent's principal conversation and this negotiation's complete turns. Reasoning excludes entries scoped to other opportunities and internal bookkeeping. The negotiator cannot fetch host state, inspect sibling opportunities, discover counterparties, or schedule an intent run.
...
The diagram shows logical ordering, not a requirement to wait for an entire principal run: v2 can start a negotiation as soon as that opportunity's instruction has been persisted. The two layers share their principal and intent; the negotiator receives scoped principal conversation entries with authorship, excluding bookkeeping and sibling opportunities.
...
5. **Negotiation reasoning:** `NegotiatorAgent.negotiate()` checks scoped principal evidence and negotiation history, then writes a permitted turn or stall in one bounded `Execute` run under its instruction and fixed role. Only the original responder may accept.
...
| Negotiation reasoning | `NegotiatorAgent.negotiate()` receives `profile`, `intent`, `conversation`, `brief`, ordered `turns`, fixed `role`, and one `opportunity`; reasons from that evidence and returns a permitted turn or stall through `Execute`. |
...
| `agents/negotiator.instructions.ts` | Source evidence, missing-input, contradiction, ask-before, and fixed-role reasoning guidance. |
...
- Both reasoning layers use the host-configured `Execute` dependency and receive runner cancellation. There is no separate negotiation decision provider.
...
Every invocation owns a temporary transcript initialized with the agent-prepared `instructions` and `prompt`, passed through unchanged. Instructions contain policy and date guidance; the prompt owns principal identity, intent and evidence. Execution identity/operation metadata stays out of that transcript. Only model-facing tool definitions go to the model; local handlers and terminal markers stay in the executor. Handlers run sequentially in returned order. Independent runs can overlap without sharing transcript state.

One step is a model response and its requested tool calls. The loop honors the agent's 8/1/3 limits and stops on no calls or a successful tool marked `terminal: true`. Terminal success skips remaining calls in that response and any further model completion; negotiation `submit_turn` and `stall` use this marker. Principal-agent tools remain nonterminal. Outputs already recorded are retained at the budget limit. Strings pass through as feedback; other results are JSON-serialized, with null for no value. Empty argument text becomes `{}`, as in v2. Unknown tools, invalid JSON, and ordinary handler errors, including terminal-tool failures, become feedback within the remaining budget; there is no automatic retry or schema validator.
...
### Grounded negotiation reasoning

One `NegotiatorAgent.negotiate()` invocation checks the evidence and writes the turn or stall through a single bounded `Execute` run. A successful output ends the run immediately; the existing three-step budget allows correction of invalid calls. Jev is not called on the runtime path. `TypeSafeClient` remains available for explicit fixed-case comparisons before reconsidering a mandatory gate.

- **Inputs:** current intent, confirmed profile fields, every human `user` or `answer` entry from this intent's H2A conversation, the current opportunity's agent H2A entries, brief, counterparty intent, and all negotiation turns labeled `our_agent` or `counterparty_agent`. H2A entries retain their kinds, chronological order, and explicit `principal` or `principal_agent` speakers. Agent entries scoped to other opportunities, bookkeeping, and unconfirmed profile fields are excluded.
- **No duplicate source fields:** intent and profile are supplied in the prompt rather than repeated in instructions. The source evidence block owns the brief, counterpart intent and full turns. Negotiation metadata carries identity, status, permissions and the numeric turn budget without repeating those fields or the latest turn as `terms`. Principal prompts reference pending question IDs; their text remains in the conversation.
- **Source authority:** explicit principal words and confirmed profile facts can support an answer omitted from the brief. A human statement made about another opportunity may establish an explicit general skill, location, or availability fact, but never interest, permission, approval, or an answer for this opportunity. Later principal answers supersede stale summaries. Agent questions, summaries, earlier A2A claims, and counterpart assertions cannot establish principal facts or grant permission.
- **Reasoning policy:** check every decision-critical requirement against the person it describes. Decline supported contradictions even if unrelated information is missing. Otherwise stall on missing required principal facts or unresolved applicable ask-before boundaries, and ask the counterpart about missing counterpart facts. Missing budget evidence does not establish a budget conflict. A human answer resolves a boundary even when the brief is stale.
- **Enforced authority:** tools and handlers constrain actions to host permissions, the standing decision, responder-only acceptance, and one output. Evidence judgments are model reasoning rather than deterministic code enforcement. The host validates final turn submission.
- **Failure and persistence:** execution failure or observed cancellation rejects instead of fabricating a stall. Existing conversation and turn writes remain authoritative; no schema or stored metadata changes. Private conversation and brief text must not be disclosed in counterpart messages.
- **Caller change:** remove `decisions` from runner and negotiator construction. Standalone `negotiate()` callers still supply `conversation` and speaker-labeled `turns`; the runner uses its existing reads. The TUI requires only `OPENROUTER_API_KEY`.
...
flowchart TD
    ...
    Context --> Reason[Check evidence and write within one Execute run]
    Permissions[Host actions, standing decision, fixed role] --> Tools[Permitted tools and handlers]
    Reason --> Tools
    Tools --> Turn[One turn]
    Tools --> Stall[Missing principal input]
    Turn --> Host[Host validates and persists]
    Stall --> PrincipalAgent[Principal agent considers a question]
```
...
- Wake context includes this intent's current opportunity records and standing context. Each opportunity carries complete ordered, speaker-labeled `turns`, `turnCount`, `maxTurns`, and `remainingTurns`, with no duplicate latest-message field. `toOpportunity()` derives the remaining shared A2A budget from the host's count and limit, clamped at zero. A single-negotiation read retains the same fields in one `BriefContext` and its authoritative `NegotiationDetail`, so orchestration can brief missing instructions and submit against the observed turn count without loading sibling opportunities. Both reads retain a separate private publication context containing authoritative match references and historical question text; these maps are not part of either agent's reasoning inputs.
- Preparing `NegotiateContext` carries profile, intent, conversation, brief, fixed role, and one opportunity's metadata and turn budget. It moves the same A2A history into top-level `turns`, excluding it from opportunity metadata. It intersects host actions with the standing decision and responder-only acceptance rule. The negotiator sends all human facts plus only this opportunity's agent context as source evidence; raw host records remain private to the runner. These context changes add no host reads, model calls, or persistence changes.
