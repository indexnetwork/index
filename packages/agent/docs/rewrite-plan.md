...
- **No duplicate source fields:** intent and profile are supplied in the prompt rather than repeated in instructions. The source evidence block owns the brief, counterpart intent and full turns. Negotiation metadata carries identity, status and permissions without repeating those fields or the latest turn as `terms`. Principal prompts reference pending question IDs; their text remains in the conversation.
...
- Preparing `NegotiateContext` carries profile, intent, conversation, brief, fixed role, one opportunity, and all A2A turns with agent authorship. It intersects host actions with the standing decision and responder-only acceptance rule. The negotiator filters the conversation before sending source evidence to execution; raw host records remain private to the runner.
