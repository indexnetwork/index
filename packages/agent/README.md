...
| **Activation / review** | A permitted trigger and the H2A work it starts. Accepted human input or explicit intent creation/broadcast/resume can activate H2A; A2A activity cannot. |
...
- Accepted `message()` / `answer()` input, explicit `wake()`, or `activate()` with `intent.created`, `intent.broadcast` (with `networkId`), or `intent.resumed` (with `lifecycleVersionMs`) activates H2A. The API host maps a committed transition to ACTIVE to `intent.resumed:<intentId>:<lifecycleVersionMs>` and validates the current lifecycle version transactionally. Duplicate or superseded resumes do not wake again. Startup, scans, content updates, A2A activity and opening the app remain observational.
