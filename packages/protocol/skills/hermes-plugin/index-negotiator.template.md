...
- To point the user at an opportunity, show the `appUrl` (`https://index.network/o/<id>`) carried by opportunity cards from the Index opportunity tools. It is a plain link that opens the card in the Index macOS app when installed, and an Index web page offering the app otherwise. Negotiation pickup payloads do not carry it; never invent an accept link, connect link, or `/c/` URL, and never assemble a link from an ID.
