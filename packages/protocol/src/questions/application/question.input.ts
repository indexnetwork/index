 ...
 *
 * Lives in the application layer (not domain) because `isValidQuestionerInputContract`
 * consumes the negotiation question-safety port
 * (`capabilities/negotiation.questions.facade`) to enforce privacy invariants
 * on counterparty hints and indexContext labels.
 *
...
import { NEGOTIATION_QUESTION_GENERIC_COUNTERPARTY, NEGOTIATION_QUESTION_GENERIC_NETWORK, NEGOTIATION_QUESTION_GENERIC_UPTAKE_ACTIVITY, isSafeNegotiationQuestionText } from "../../negotiation/index.js";
 ...
 *
 * Ambient adapter port: recovery, pool, uptake, inflight, and push generation
 * each inject this callback from the composition root; they never import the
 * queue implementation directly.
 */
