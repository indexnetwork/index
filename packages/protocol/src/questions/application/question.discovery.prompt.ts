/**
 * questions/application/question.discovery.prompt — discovery-mode prompt for QuestionerAgent.
 *
 * System prompt constant and a pure string-building `buildDiscoveryQuestionPrompt`
 * that assembles the user message from a `DiscoveryQuestionInput`.
 *
 * Pure: no I/O, no LLM call. QuestionerAgent (via the `discovery` preset in
 * `question.presets.ts`) orchestrates this module + an LLM client.
 *
 * IND-547: canonical home — previously questioner/questioner.discovery.prompt.ts.
