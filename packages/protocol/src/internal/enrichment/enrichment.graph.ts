...
// ─────────────────────────────────────────────────────────
export async function scrapeNode(state: EnrichmentState, deps: EnrichmentGraphDeps) {
...
// ─────────────────────────────────────────────────────────
export async function autoGenerateNode(state: EnrichmentState, deps: EnrichmentGraphDeps) {
...
// ─────────────────────────────────────────────────────────
export async function decomposePremisesNode(state: EnrichmentState, deps: EnrichmentGraphDeps) {
  return timed("ProfileGraph.decomposePremises", async () => {
    ...
    try {
      ...
      const result = await deps.premiseDecomposer.invoke(
        state.input,
      ...
      for (const premiseId of retractionIds) {
        if (!deps.database.updatePremise) break;
        try {
