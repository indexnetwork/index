      ...
      try {
        if (actionType === 'create') {
          ...
          deps.intentFollowUp?.onIntentSaved({
            ...
            logger.error('Failed to run intent follow-up', { intentId: created.id, error: err })
          );
          ...
          if (updated) {
            deps.intentFollowUp?.onIntentSaved({
              ...
              logger.error('Failed to run intent follow-up', { intentId: updateAction.id, error: err })
            );
          }
