...
import type { ActiveIntent, Id, IntentRecord, Opportunity } from '../../../platform/database.js';
...
describe('negotiateExistingOpportunity', () => {
    ...
    const result = await negotiateExistingOpportunity(
      ...
      executeNegotiation,
      {
        ...
        continuation: {
          ...
          counterpartyBinding: { kind: 'intent', id: 'different-intent' },
  ...
  test('isolates both sides on an exact continuation and skips unrelated active-intent reads', async () => {
    ...
    await negotiateExistingOpportunity(
      ...
      },
      {
        ...
        continuation: {
          ...
          opportunityUpdatedAt: new Date().toISOString(),
          counterpartyUserId: 'counterparty',
          counterpartyBinding: { kind: 'intent', id: 'intent-counterparty' },
          successorTaskId: 'task-2',
          conversationId: 'conversation-1',
          token: 'token',
          fence: 1,
  ...
  test('sends no unrelated fallback for an actor with no exact opportunity-bound intent', async () => {
        const captured: Array<{ source: string[]; candidate: string[] }> = [];
    const persistedOpportunity = opportunity({
      actors: [
