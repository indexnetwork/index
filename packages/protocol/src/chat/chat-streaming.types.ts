...
/**
 * One question generated by a rejecting counterpart in an orchestrator-inline
 * negotiation. Surfaced to the user as a chat card so they can enrich the
 * source intent and re-run discovery.
 */
export interface ClarificationQuestion {
  /** Stable id used by the answer endpoint to look up the pending row. */
  id: string;
  /** Counterpart user the question came from. */
  candidateUserId: string;
  /** Opportunity row tied to the underlying negotiation, when present. */
  opportunityId?: string;
  /** Network the negotiation ran on (used to scope re-runs). */
  networkId?: string;
  /** Counterpart's display name, when known — rendered as a chip on the card. */
  sourceAgentName?: string;
  /** Verbatim question from the rejecting turn. */
  question: string;
  /** Index relevancy score for this candidate (used to rank cards). */
  relevancyScore: number;
}

/**
 * Sent at the end of an orchestrator-inline discovery turn when one or more
 * high-relevancy candidates rejected on missing-but-fillable info. The frontend
 * renders these as a paginated card so the user can enrich the source intent.
 */
export interface ClarificationRequestEvent extends ChatStreamEventBase {
  type: "clarification_request";
  /** The source intent the user can enrich (null when no intent was anchored to this query). */
  intentId?: string;
  /** The original search query — surfaced in the card header for context. */
  searchQuery?: string;
...
export function createClarificationRequestEvent(
  sessionId: string,
  payload: Omit<ClarificationRequestEvent, "type" | "sessionId" | "timestamp">,
): ClarificationRequestEvent {
  return createStreamEvent<ClarificationRequestEvent>(
    "clarification_request",
    sessionId,
    payload,
  );
