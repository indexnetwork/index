...
```sh
bun install
bun run --cwd packages/cli dev login # once, or set INDEX_SESSION_TOKEN
bun run agent:tui
```

`INDEX_API_URL` selects the server origin (default: the CLI login's origin or `http://localhost:3001`). A session token is required. **An agent API key is not an owner credential:** with a selected executor it speaks as the agent in the agent DM. This TUI will not use one. Like macOS, it reads existing owner data; it never seeds or resets it.
