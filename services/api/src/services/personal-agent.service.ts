  ...
  async start(): Promise<void> {
    ...
    this.subscriber.on('pmessage', (_pattern, channel: string, raw: string) => {
      ...
      try {
        ...
        // Content invalidations can precede assignments; follow-up wakes the session when they are complete.
        if (!['intent.lifecycle', 'intent.pursuit', 'agent.configuration'].includes(type)) return;
