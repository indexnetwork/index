...
// Delegations and retirements are private agent records, not chat entries or unread messages.
const visibleMessage = sql`NOT (${schema.messages.role} = 'agent' AND ${schema.messages.senderId} = ${SYSTEM_AGENT_ID}
  AND (coalesce(${schema.messages.metadata}, '{}'::jsonb) ? 'principalDelegation'
