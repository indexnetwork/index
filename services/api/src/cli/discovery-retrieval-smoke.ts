...
async function attest(target: SmokeTarget, controlPlane: NeonControlPlane): Promise<void> {
  ...
  if (branch.id !== target.branchId || !branch.name.startsWith('eval-discovery-retrieval-') || branch.parentId !== parent.id || branch.primary || !branch.expiresAt || Number.isNaN(Date.parse(branch.expiresAt)) || Date.parse(branch.expiresAt) <= Date.now()) throw new Error('Smoke branch is not a live disposable child of the protected base');
