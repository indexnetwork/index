...
async function main(): Promise<void> {
  ...
  const { PrincipalRecordsDatabaseAdapter } = await import('../adapters/principal-records.database.adapter');
  ...
  try {
    ...
    if (args.command === 'question') {
      ...
      const records = new PrincipalRecordsDatabaseAdapter(recipient.id, intent.id);
      try {
        await records.start();
        const current = await records.read();
        const saved = await records.write({ negotiations: [], delegations: [], retiredQuestionIds: [], messages: [{
          id: crypto.randomUUID(), createdAt: new Date().toISOString(), kind: 'question',
          questionId: question.id, batchId: crypto.randomUUID(), text: question.question, options: question.options, matches: [],
        }] }, current.version);
