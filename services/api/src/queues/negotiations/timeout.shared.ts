...
function actingUserContext(userId: string): UserNegotiationContext {
  return { id: userId, intents: [], profile: {} };
}
...
export async function runResumableTimeoutFallback(params: {
  ...
  if (execution.status === 'pending') {
    ...
    const turn = await invokeNegotiator({
      ownUser: actingUserContext(activeUserId),
...
export async function runTimeoutFallback(params: {
  ...
  const ownUserCtx: UserNegotiationContext = actingUserContext(activeUserId);
