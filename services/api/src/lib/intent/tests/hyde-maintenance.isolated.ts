/**
 * Unit tests for HydeMaintenance (cron-based cleanup and refresh). Uses injected database only.
 */
...
describe('HydeMaintenance', () => {
  describe('constructor', () => {
    it('uses default adapter when no deps', () => {
      const queue = new HydeMaintenance();
    ...
    it('uses provided database when deps given', async () => {
      ...
      const db = {
        ...
        getStaleHydeDocuments: async () => [] as Awaited<ReturnType<HydeMaintenanceDatabase['getStaleHydeDocuments']>>,
        getIntentForIndexing: async () => null as unknown as Awaited<ReturnType<HydeMaintenanceDatabase['getIntentForIndexing']>>,
        deleteHydeDocumentsForSource: async () => 0,
      };
      const queue = new HydeMaintenance({ database: asHydeDb(db) });
  ...
  describe('refreshStaleHyde', () => {
    ...
    it('skips doc when sourceId is missing', async () => {
      const db = {
        ...
        ] as Awaited<ReturnType<HydeMaintenanceDatabase['getStaleHydeDocuments']>>,
        getIntentForIndexing: async () => null as unknown as Awaited<ReturnType<HydeMaintenanceDatabase['getIntentForIndexing']>>,
        deleteHydeDocumentsForSource: async () => 0,
      };
      const queue = new HydeMaintenance({ database: asHydeDb(db) });
    ...
    it('deletes HyDE when intent not found and increments refreshed only for valid', async () => {
      ...
      const _queue = new HydeMaintenance({ database: asHydeDb(db) });
      ...
      const getStaleHydeDocuments = mock(async () =>
        [{ sourceId: 'i1', sourceType: 'intent', strategy: 'mirror' }] as Awaited<
          ReturnType<HydeMaintenanceDatabase['getStaleHydeDocuments']>
        >
      );
      const db2 = {
        deleteExpiredHydeDocuments: async () => 0,
      ...
      const queue2 = new HydeMaintenance({ database: asHydeDb(db2) });
    ...
    it('preserves stale HyDE and never invokes the graph for inactive or ownerless intents', async () => {
      ...
      const db = {
        ...
        getIntentForIndexing: async (sourceId: string) => rows[sourceId as keyof typeof rows] as Awaited<
          ReturnType<HydeMaintenanceDatabase['getIntentForIndexing']>
        >,
        deleteHydeDocumentsForSource,
      };
      const queue = new HydeMaintenance({ database: asHydeDb(db), invokeHyde });
    ...
    it('still regenerates stale HyDE for active legacy and explicit-active intents', async () => {
      ...
      const db = {
        ...
        getIntentForIndexing: async (sourceId: string) => ({
          ...
          archivedAt: null,
        }) as Awaited<ReturnType<HydeMaintenanceDatabase['getIntentForIndexing']>>,
        deleteHydeDocumentsForSource: async () => 0,
      };
      const queue = new HydeMaintenance({ database: asHydeDb(db), invokeHyde });
