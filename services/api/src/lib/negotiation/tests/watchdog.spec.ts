...
import { MAX_WATCHDOG_ATTEMPTS, NegotiationWatchdog, SUBMITTED_STALE_AFTER_MS, WORKING_STALE_AFTER_MS } from '../watchdog';
...
describe('NegotiationWatchdog', () => {
  it('pauses stale submitted tasks straight through the negotiation graph', async () => {
    const deps = makeDeps();
    const queue = new NegotiationWatchdog({ ...deps, clock: () => now });
  ...
  it('does nothing when the task changed state after the stale-list read', async () => {
    ...
    const queue = new NegotiationWatchdog(deps);
  ...
  it('start is idempotent', async () => {
    const queue = new NegotiationWatchdog();
    await queue.start();
    await queue.start();
    await queue.close();
  });
