/**
 * IND-406 — NegotiationReflect job handlers (unit tests, deps injected).
 *
 * Pins:
...
import { NegotiationReflect, type ReflectJobData } from '../reflect';
...
function mkQueue(opts?: {
  ...
  const queue = new NegotiationReflect({
    conversations: {
...
describe('NegotiationReflect', () => {
  const queues: NegotiationReflect[] = [];
