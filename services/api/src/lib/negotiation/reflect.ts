 ...
 *
 * Two job kinds:
 * - `reflect`: started by the negotiation graph's finalize node (via the
 *   injected `ReflectEnqueueFn` — the protocol package has no host scheduler).
 ...
 * finalized before the job runs, and the graph starts reflection fire-and-forget.
 *
...
/**
 * Reflection: delayed chat reflection plus a decay cron.
 */
export class NegotiationReflect {
  ...
  async addReflectJob(data: ReflectJobData): Promise<void> {
    await this.processJob('reflect', data);
  }
  ...
  async scheduleChatReflect(data: ChatReflectJobData): Promise<void> {
    ...
    const timer = setTimeout(() => {
      ...
      void this.processJob('chat_reflect', data).catch((err) => {
        this.logger.error('Chat reflect failed', { sessionId: data.sessionId, error: err });
      });
    }, CHAT_REFLECT_DELAY_MS);
    timer.unref?.();
  ...
  async close(): Promise<void> {
    for (const timer of this.chatReflectTimers.values()) clearTimeout(timer);
    this.chatReflectTimers.clear();
  }
