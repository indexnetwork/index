 ...
 * lost.
 */
export class NegotiationWatchdog {
  ...
  constructor(deps: NegotiationWatchdogDeps = {}) {
    this.deps = deps;
    this.logger = deps.logger ?? log.job.from('NegotiationWatchdog');
  ...
  async start(): Promise<void> {
    ...
    this.task = cron.schedule('*/5 * * * *', () => {
      this.sweep().catch((error) => {
        this.logger.error('Negotiation watchdog sweep failed', { error });
      });
    });
...
export const negotiationWatchdog = new NegotiationWatchdog();
