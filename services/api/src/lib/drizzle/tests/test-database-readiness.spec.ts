...
describe('test database readiness', () => {
  ...
  test('requires current tables before database-backed fixtures run', () => {
    expect(REQUIRED_TEST_DATABASE_OBJECTS).toContain('public.intent_proposals');
    expect(REQUIRED_TEST_DATABASE_OBJECTS).not.toContain('public.hermes_authorizations');
    expect(REQUIRED_TEST_DATABASE_OBJECTS).not.toContain('public.hermes_agent_credentials');
  });
