 ...
 *
 * @remarks
 * Call {@link PremiseCascade.startCrons} from the protocol server.
 */
export class PremiseCascade {
  ...
  /**
   * Start a profile-decomposition run. Deduplicated by user so rapid
  ...
  startCrons(): void {
    ...
    this.expiryLogger.info('📅 Expiry check scheduled (every hour)');
  }
