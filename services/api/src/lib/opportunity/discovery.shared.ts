...
/**
 * Graph DB shape the opportunity/HyDE graphs require.
 */
export type OpportunityGraphDb = OpportunityGraphDatabase & HydeGraphDatabase;

/** Runtime deps shared by opportunity discovery. */
 ...
 * Identical across discovery entry points, so it lives here once.
 */
 ...
 *
 * Encapsulates graph assembly + invocation, `result.error` handling, and the
 * candidates/opportunities (and optional trace)
...
export async function runOpportunityDiscovery<TOpts extends OpportunityInvokeOptions>(params: {
  ...
  /** Human label for logs, e.g. `'FromIntent'`. */
  label: string;
  /**
   * Label for the thrown fallback error message, e.g. `'from-intent'`. Kept
   * distinct from `label` so the thrown message stays lowercase-dashed.
   * Defaults to `label`.
   */
  errorLabel?: string;
