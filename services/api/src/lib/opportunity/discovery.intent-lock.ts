 ...
 *
 * Persistence already tolerates overlap (pair dedup IND-166, the
 * `final_atomic_conflict` re-check, the intent-scoped atomic create+expire),
 * so this guard is not a correctness gate; it stops two runs spending LLM
 * and embedder budget on the same intent at the same time. A short-TTL Redis
 * lock keyed by intentId (`SET NX PX` + holder-checked release), with an
 * in-process map in the hermetic test baseline so specs exercise the real
 * acquire→defer path without Redis.
 */
 ...
 * IntentDiscovery instance in a process sees the same locks.
 */
