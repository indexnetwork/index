 ...
 * Non-deprecated: lib/*, controllers/, adapters/, crons/, and root main.ts only.
...
export function isDeprecatedSource(sourcePath: string): boolean {
  ...
  if (normalized.startsWith('crons/')) return false;
  return true;
}
...
export const log = {
   ...
   * crons = PascalCase class name ('OpportunityExpirationCron'); adapters = '<name>.adapter'; guards = '<name>.guard';
