...
### Added
- Add post-discovery no-opportunity recovery refinement for exact recipient-owned active intents (IND-506). Both authoritative completion paths enqueue a privacy-minimal job on the existing Questioner worker; a focused service suppresses recovery when canonical exact-trigger actionability exists, reduces safely validated rejected negotiations to a bounded aggregate count, and persists at most one ordinary intent question per material fingerprint behind advisory-lock lifecycle/fingerprint/actionability rechecks plus migration `0105`'s all-status expression unique index. Answer and material-edit paths system-void stale recovery rows without emitting stale reactions, REST strips every recovery internal, and pool questions retain independent budget/novelty behavior.
