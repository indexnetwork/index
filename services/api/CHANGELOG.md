...
- Add exact negotiation-question admission/read/settlement routing (IND-507): API/DB validation proves the authenticated recipient's exact owned ACTIVE fingerprint-equal signal, assignment, live non-personal membership, opportunity actor binding, network, and purpose-compatible task state before generation and under ordered locks before insert/settlement. Migration `0106_add_negotiation_question_provenance_index` consolidates uptake/ordinary/inflight idempotency into one versioned JSON-expression constraint across statuses; stale/legacy rows fail closed, inflight answer/dismiss/timeout claim only the stamped task, and REST/MCP strip all provenance internals.
