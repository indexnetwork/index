...
- Default-off Lens B outcome feedback capture (IND-434): verified explicit human
  ...
  is gated on session-authenticated provenance from the controller boundary, so
  agent/API-key and system status mutations are never recorded as preferences.
  Shadow mining re-validates intent ownership, active lifecycle, and fingerprint
  immediately before running, deduplicates related opportunities by canonical
  counterpart identity, and emits redacted, threshold-safe aggregate telemetry
