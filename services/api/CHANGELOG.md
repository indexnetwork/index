...
- **BREAKING: exhaustive intent scoring with bounded automatic openings.** `POST /intents/:id/discover` accepts no body or `{}` and returns `{ opportunities: [{ opportunityId }] }` for newly opened negotiations. TypeSafe scores every eligible public intent/network pair with no pass/fail threshold or `0.8` cutoff; discovery returns all still-eligible scores sorted descending. Automatic opening walks that ranking until up to 10 new negotiations per intent per matching run are created or candidates are exhausted. Existing/reused, terminal, and unavailable sessions do not consume the new-opening budget, and terminal sessions still require deliberate reopening. There are no queries, embedding retrieval, caller picks, or model-selected/skipped stage. Remove `POST /intents/:id/opportunities`; old query/limit/picks bodies are rejected. Move the optional `executorId` fence to discovery, retain per-pair atomicity and no automatic reopen, and validate both evaluated payloads under the opening lock. Persist the TypeSafe score, reasoning and evaluation evidence. Failed matching or writes propagate without blind retries; previously committed openings remain valid. Intent lifecycle embedding generation stays unchanged.
- Hosted reviews and external `agentv2` runs automatically request matching after persisting current principal decisions, including silent plans. Hosted new sessions copy the current standing brief unchanged; no agent selects or skips scored pairs. External matching runs after questions/notes/retirements are durable and schedules every opened negotiation without model-driven retries. Propagate request cancellation through checks and uncommitted openings; exhaustive REST matching is exempt from the ordinary idle timeout.
- Activate matching on versioned `intent.revised` receipts after material payload/summary edits. Validate each hosted revision against the current locked intent, and keep generic `intent.updated` invalidations from creating wake loops. External runners also process revision and broadcast events.
...
### Migration
- Set server-side `TYPESAFE_API_KEY` before starting the API. Matching uses pinned `jev-1.13.0` for exhaustive scoring without a pass cutoff; two public intent payloads and permitted shared network context are sent to TypeSafe. Scores order the bounded automatic openings, not consent or a calibrated probability of agreement. No matching schema migration or automatic historical backfill is required.
