...
import { API_URL, JWT_AUDIENCE } from '../src/lib/betterauth/betterauth';
...
describe('jwtVerify claim validation', () => {
  it('accepts a token with correct iss and aud', async () => {
    ...
    const token = await signToken(privateKey, { iss: API_URL, aud: JWT_AUDIENCE });
  ...
  it('rejects a token missing aud', async () => {
    const { privateKey, JWKS } = await makeTestJWKS();
    const token = await signToken(privateKey, { iss: API_URL });
    let err: unknown;
    try { await jwtVerify(token, JWKS, { issuer: API_URL, audience: JWT_AUDIENCE }); } catch (e) { err = e; }
    expect(err).toBeInstanceOf(joseErrors.JWTClaimValidationFailed);
  });

  it('rejects a token with wrong aud', async () => {
    const { privateKey, JWKS } = await makeTestJWKS();
    const token = await signToken(privateKey, { iss: API_URL, aud: `${JWT_AUDIENCE}-wrong` });
    let err: unknown;
    try { await jwtVerify(token, JWKS, { issuer: API_URL, audience: JWT_AUDIENCE }); } catch (e) { err = e; }
    expect(err).toBeInstanceOf(joseErrors.JWTClaimValidationFailed);
  });
  ...
  it('rejects a token with wrong iss', async () => {
    const { privateKey, JWKS } = await makeTestJWKS();
    const token = await signToken(privateKey, { iss: `${API_URL}-wrong`, aud: JWT_AUDIENCE });
    let err: unknown;
    try { await jwtVerify(token, JWKS, { issuer: API_URL, audience: JWT_AUDIENCE }); } catch (e) { err = e; }
    expect(err).toBeInstanceOf(joseErrors.JWTClaimValidationFailed);
  });
