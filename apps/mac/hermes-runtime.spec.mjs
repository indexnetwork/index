...
test('fails closed on navigation and externalizes only user-activated web or mail links', () => {
  ...
  expect(policy).toContain('openExternally(navigationAction.request.url)');
  expect(policy).toContain('decisionHandler(.cancel)');
  expect(main).toContain('case "http", "https", "mailto": return true');
  expect(main).toContain('NSWorkspace.shared.open(url)');
