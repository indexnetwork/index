...
[[ "$#" -eq 6 ]]||prior_error "usage: JSON CMS FINAL_DIR SHA256SUMS RELEASE_TAG REPOSITORY"
metadata="$1";cms="$2";FINAL_DIR="$3";sums="$4";release_tag="$5";repository="$6"
...
[[ "$release_tag" =~ ^v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$&&"$repository" =~ ^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$ ]]||prior_error "historical tag/repository authority is invalid"
...
bun - "$metadata" "$FINAL_DIR" "$sums" "$release_tag" "$repository" <<'JS'
const [path,dir,sumsPath,tag,repository]=process.argv.slice(2),bytes=await Bun.file(path).text();let value;try{value=JSON.parse(bytes)}catch{throw new Error("metadata is not JSON")}
...
if(!sem.test(value.releaseVersion)||tag!==`v${value.releaseVersion}`||!/^[1-9]\d*$/.test(value.buildNumber)||!/^[0-9a-f]{40}$/.test(value.commit)||!Array.isArray(value.artifacts)||value.artifacts.length!==2)throw new Error("metadata identity is noncanonical");
