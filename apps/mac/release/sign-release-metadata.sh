...
publish_owned_noreplace() { local source="$1" destination="$2"; ln "$source" "$destination"; rm -f -- "$source"; }
sign_release_metadata() (
  ...
  publish_owned_noreplace "$candidate" "$cms" || metadata_sign_error "CMS no-clobber publication failed"; candidate_identity=""
)
sign_release_metadata "$@"
