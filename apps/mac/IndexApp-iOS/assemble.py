...
Mirror of the desktop IndexApp assembler, pointed at the mobile sources
(src/index-mobile.html + src/index-mobile/*.jsx). Inlines the vendored
...
html = (SRC / "index-mobile.html").read_text()
...
# 2) Replace each <script type="text/babel" src="index-mobile/NAME.jsx"></script> by inlined JSX.
def babel_sub(m):
    rel = m.group(1)
    return inline_script(SRC / rel, attrs='type="text/babel"')

html = re.sub(
    r'<script\s+type="text/babel"\s+src="(index-mobile/[^"]+\.jsx)"></script>',
    babel_sub,
    html,
)

# Sanity: no remaining external src references into our local tree.
leftover = re.findall(r'src="(index-mobile/[^"]+)"', html)
if leftover:
