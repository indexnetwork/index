...
cleanup_release_transaction() {
  local transaction="$1" promoted="$2" status="$3"
  if (( status != 0 )); then
    [[ "$promoted" == 1 ]] && rm -rf "$FINAL_DIRECTORY"
    rm -rf "$transaction"
  fi
  return "$status"
}
release_main() (
  local final_parent transaction app_dmg connector_dmg promoted=0
  ...
  trap 'status=$?; cleanup_release_transaction "$transaction" "$promoted" "$status"' EXIT
  ...
  notarize_dmg_transaction "$app_dmg"; notarize_dmg_transaction "$connector_dmg"
  ...
  if promote_release_set "$transaction" "$FINAL_DIRECTORY"; then
    promoted=1
  else
    [[ -e "$FINAL_DIRECTORY" && ! -e "$transaction" ]] && promoted=1
    return 1
  fi
