...
html = (SRC / "halo-amiga.html").read_text()
...
# 2) Replace each <script type="text/babel" src="halo-amiga/NAME.jsx"></script> by inlined JSX.
def babel_sub(m):
    rel = m.group(1)
    return inline_script(SRC / rel, attrs='type="text/babel"')

html = re.sub(
    r'<script\s+type="text/babel"\s+src="(halo-amiga/[^"]+\.jsx)"></script>',
    babel_sub,
    html,
)
