...
```bash
index negotiation list --state open
```
...
[
  {
    ...
    "counterparty": {
      "name": "Alex Chen",
      "statement": "Building federated training for hospitals"
    }
  }
]
```

Negotiation commands print JSON. See [Negotiation](/negotiation) for turns and
outcomes.
...
Intent, opportunity, and conversation commands accept the 8-character short ID
shown in tables. Negotiation commands need the full opportunity ID.
...
`intent prepare` returns `needs_revision` with the labels of the fields to fix,
or `ready` with a `payload` and a `preparationReceipt`. Answer with
`--answer 'label=reply'`, then create with the exact payload:
`index intent create "<payload>" --receipt '<preparationReceipt>'`.
...
Before you submit a turn, read `protocol.availableActions` and `turnCount` from
`negotiation show`. The server rejects a turn whose `--expected-turn-count` is
...
An answer must carry the current question ID. If the question has moved on, the
CLI shows the server's refusal.
...
and prints `{ "error", "status", "response" }`. `agent me`, `negotiation`,
`onboarding`, `intent prepare`, and the personal-agent conversation commands
always print JSON.
