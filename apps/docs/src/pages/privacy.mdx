...
3. **[Agent and agent](#agent-and-agent)** — what each side learns during a
   negotiation.
4. **[Each person and their agent](#each-person-and-their-agent)** — what each
   person sees, and what needs their yes.
...
**[Appropriateness](/privacy/appropriateness)** can be used to automate these decisions: how open an intent
...
Intent visibility and network scope are enforced by the shared
...
The negotiator agent can use **[Appropriateness](/privacy/appropriateness)** information
...
## Each person and their agent

Each person sees the same opportunity through their own agent, and nothing
becomes a connection without a yes.

- **Per-viewer card:** Each person reads a card written for their role: why
  this pair, and their own next action. The card carries no embeddings or
  scores. Private context can shape what a person sees without being revealed
  to the other side.
- **Access:** Only the actors on an opportunity can see it.
- **Consent:** When the agents agree, the opportunity becomes `pending`. Each
  person's accept is a separate act. Silence, errors, and timeouts are never
  consent, and a later yes cannot override an earlier refusal.

What this makes possible: expiry on consent. An actor's part in an
[opportunity](/opportunity) can expire, and that expiry is the way out of the
opportunity, into a conversation or a further event.

## Planned

- **Federated hosting:** A community runs its own host, so its membership and
  corpus stay on infrastructure it controls.
- **Network TEE:** Discovery and negotiation run inside a trusted execution
  environment, so whoever operates the host cannot read the corpus.
- **Incognito negotiation:** Incognito intents are negotiated inside the
  network TEE, so their content opens only as the negotiation passes its
  checks. See [Incognito negotiation](/privacy#incognito-negotiation).
