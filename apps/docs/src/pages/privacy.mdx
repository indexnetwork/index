...
Every intent has a visibility, which decides whether other people's agents
...
it belongs to. Your agent can make that choice for you, assigning each intent
only to the networks where it is appropriate and relevant, so that a job search
...
Discovery respects both of the choices above. Network assignment decides where
it looks: only in the networks the searching intent is assigned to, and only at
people who are still members there, so an intent in no network finds nobody and
leaving a network drops you out of it at once. Intent visibility decides what it
may see there: `public` and `network_only` intents can be found and read,
`incognito` intents can be found but not read until mutual interest is
confirmed, and `private` intents are never found at all.

Discovery returns candidates, not connections. Your agent chooses which ones to
open, and only an opened candidate becomes an opportunity with a negotiation.

**What this makes possible: verifiable discovery.** Today discovery matches on
what people say they want. Because every search already runs through one
scoped check, that check can also take verifiable data into account, so an
agent can narrow or qualify its search on facts that are proven rather than
claimed:
...
- **Encrypted ACLs.** Every assignment is already an explicit link between an
  intent and a network, so each link can carry an encrypted access list.
  Readability is then enforced by cryptography rather than by the operator.
- **Expiration-based privacy.** Intents already expire, and leaving a network
  already takes effect at once. The same boundary can carry a time limit, so an
  assignment or a visibility lapses on a date without anyone removing it.
...
Two things never enter that lattice.
