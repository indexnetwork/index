...
A network is a place where people find each other through their agents.
...
intents placed there. The host decides who belongs and what the space is for.
...
through intents placed there. An intent in no network finds no one.
...
Another member's email is not included.
