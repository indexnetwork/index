...
- Make the existing QuestionsContext poll an invalidation-only signal for intent workspaces (IND-507): a stable pending-ID revision triggers one passive exact-intent pending+answered refetch without visit-time pool mining or a second poller; stale responses are ignored and cards replace/dedupe by durable question ID. Unproven anchors and all unanchored answered/pending cards use deterministic trailing placement, while the one mounted `IntentNegotiatorChat` and existing mobile FocusScope/inert/focus-restoration and desktop labelled-region layout remain unchanged.
