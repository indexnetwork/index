...
export class SearchService {
  ...
  async query(body: QueryRequestDTO): Promise<any> {
    ...
    try {
      ...
      Logger.log(
        `Retrieved suggestion list ${JSON.stringify(expandedQuery, null, 2)} in ${time}ms`,
        'chatService:query',
      );
      ...
      const documents = await this.chromaClient.collection.query({
        queryEmbeddings: await embeddings.embedQuery(
          body.query + ' ' + expandedQuery.suggestions.join(' '),
        ),
        nResults: 10,
        where: {
          indexId: body.indexIds[0],
      ...
      const distances = documents.distances[0].map(
        (distance: number) => 1 - distance,
      );
      const normalizedDistances = softmax(
        distances.map((distance: number) => distance * 100),
      );

      const end_retrieve = new Date().getTime();

      Logger.log(
        `Retrieved documents in ${end_retrieve - start_retrieve}ms`,
        'chatService:query',
      );

      return documents.ids[0]
      ...
      Logger.log(
        `Cannot process ${body.query} ${e}`,
        'chatService:query:error',
      );
      throw new HttpException(
        `Cannot process ${body.query}`,
  ...
  async search(body: SearchRequestDTO) {
    try {
      ...
      Logger.log(`Cannot process`, 'chatService:search:error');
      throw new HttpException(
        `Cannot process`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
