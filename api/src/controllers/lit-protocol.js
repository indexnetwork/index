...
import {
  getNftMetadataApi,
  getCollectionMetadataApi,
  getENSProfileByWallet,
  ...
  conditions = await Promise.all(
    conditions.map(async (c) => {
      ...
      if (value.standardContractType) {
        ...
        let collectionMetadata = await getCollectionMetadataApi(
          value.chain,
        ...
        if (collectionMetadata) {
          ...
          if (value.metadata.tokenId) {
            let tokenMetadata = await getNftMetadataApi(
              value.chain,
              value.contractAddress,
...
export const postAction = async (req, res, next) => {
  ...
  const generatedCID = await generateLITAction(req.body, definition);
