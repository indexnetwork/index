...
A successful push has exactly two surfaces: the delivered row joins the Personal Agent sidebar count, and one deterministic assistant line is inserted into the recipient's negotiator session pinned to the push's own intent, with a link to `/i/:intentId`. It never creates a global Questions row, injected unscoped card, toast, modal, or separate notification. Because the target is the intent-pinned session, delivery resolves (and may create) that session; when the intent has been archived, deleted, or is no longer the recipient's, the claim is suppressed rather than retried. The message remains historical after resolution; the badge clears naturally because counts require pending status and the authoritative internal `pushedAt` delivery stamp. Internal pool snapshots and push claim/delivery metadata are stripped from REST and MCP payloads.
