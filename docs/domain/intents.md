...
This behavior is independently gated: `POOL_QUESTIONS_MODE` controls question generation/application, `POOL_QUESTIONS_PUSH` additionally controls proactive delivery (default off), `POOL_QUESTIONS_STAMP_NEWBORN` controls pre-insert preference stamping (default off), and `POOL_QUESTIONS_RANKING` controls whether stored adjustments affect read-time ordering. With ranking off, feed ordering remains unchanged.
