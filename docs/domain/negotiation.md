...
Flow: the `ask_user` turn is persisted → the task transitions to `input_required` (an A2A state previously unused by negotiations) → the draft is refined by the questioner's `negotiation_inflight` preset and delivered through the pending-questions UI → a **24 h answer window** is armed (`NEGOTIATION_ASK_USER_WINDOW_MS` overridable). The graph exits at the turn boundary, so chat-triggered runs never block a stream on a question — the resume is always an async continuation. The question carries a server-only versioned provenance envelope binding the exact recipient, that recipient's exact opportunity actor intent, opportunity, non-personal network, and task. The API proves current ownership, ACTIVE lifecycle, assignment, membership, actor visibility, task state, and the canonical payload+summary fingerprint before generation and again under locks immediately before insertion; missing, ambiguous, foreign, or drifted bindings fail closed.

- **Answer in time**: one locked transaction revalidates provenance, stores the answer on the question, appends the established shared `metadata.userAnswers` context, and conditionally closes only the stamped `input_required` task (`canceled`, superseded). Post-commit work cancels that task's timer and enqueues one continuation. The asker speaks again (an `ask_user` turn does not pass the floor) with the answer in its context.
- **Dismissal or window expiry**: the same exact task/question cohort is atomically claimed once, dismissed, and resumed with a **conservative default** instructing no disclosure or commitment. A newer task for the same opportunity is never selected. Answer-vs-timeout races have one winner; stale intent/opportunity/task state produces no shared mutation or continuation.
