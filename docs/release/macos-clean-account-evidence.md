...
The schema-v3 records bind the same exact release version, commit, **both** final DMG SHA-256 values, private candidate-manifest SHA-256, GitHub attestation URL, and macOS floor. Each exact canonical JSON record must use recursively lexicographically sorted object keys, compact `JSON.stringify` encoding, and exactly one final newline, and must be carried in an opaque CMS signed by its architecture's reviewed certificate pin. The arm64 pin authenticates only an `architecture: "arm64"` record and the x86_64 pin only an `architecture: "x86_64"` record. The two pins, signer certificates, approvers, and architectures must all be distinct; unsigned or self-asserted records are never approval authority. The remaining closed schema requires macOS 13+, quarantine/Gatekeeper proof, standalone connector operation with Index absent, all six canonical capability families, negotiation pickup/respond/consultation and Index fallback, near-expiry reconnect, disconnect/revocation, plaintext migration, zero secret-scan matches, uninstall/reinstall, and nonempty screenshot/log SHA-256 evidence lists.
