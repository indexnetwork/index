...
1. Obtain separate incident authorization for a durable ingress/control-plane **admission fence** independent of the emergency database transaction. Enable it across every replica and queue consumer so it denies all new Hermes authority creation, approval, exchange, activation, legacy preparation, and Hermes runtime selection, specifically:
   ...
   - Hermes selection through `PUT /api/agent-runtime`

   Also stop Hermes client rollout and scheduled negotiator launches. The fence must be durable across API deployments/restarts, block every equivalent ingress and control-plane path, and must not depend on transaction locks. Then drain all in-flight requests for these paths and use credential-free probes to prove denial before the dry-run. Record only fixed route categories and denial/pass states. Do not proceed if any path can still admit work. The separately incident-authorized fence remains held through zero verification and older-binary restoration; release it only under a later, separate re-enablement authorization.
...
The emergency command defaults to dry-run. Begin only after the admission fence is verified. The dry-run is safe to repeat and must occur immediately before confirmation. Confirmation requires the exact opaque plan ID and the exact installation count from that plan; a changed snapshot fails without partial mutation.
...
jq -e '
  ...
  and ([.installations, .credentials, .permissions, .owners]
    | all(type == "number" and floor == . and . >= 0 and . <= 9007199254740991))
' "$plan_file" >/dev/null
...
plan_id="$(jq -er '
  ...
  | .planId
' "$plan_file")"
expected_installations="$(jq -er '.installations | tostring' "$plan_file")"
expected_credentials="$(jq -er '.credentials | tostring' "$plan_file")"
expected_permissions="$(jq -er '.permissions | tostring' "$plan_file")"
expected_owners="$(jq -er '.owners | tostring' "$plan_file")"

# Enforce canonical non-negative decimal safe integers before mutation.
for expected_count in \
  "$expected_installations" "$expected_credentials" "$expected_permissions" "$expected_owners"; do
  [[ "$expected_count" =~ ^(0|[1-9][0-9]*)$ ]]
  (( expected_count <= 9007199254740991 ))
done
...
jq -e \
  --arg planId "$plan_id" \
  --argjson installations "$expected_installations" \
  --argjson credentials "$expected_credentials" \
  --argjson permissions "$expected_permissions" \
  --argjson owners "$expected_owners" '
  (keys == ["audience", "auditReceipts", "credentials", "credentialsRevoked", "durationMs", "installations", "installationsDisconnected", "owners", "permissions", "permissionsRemoved", "planId", "reason", "receiptId", "selectedPaused"])
  and .audience == "hermes-agent"
  and .planId == $planId
  and .receiptId == $planId
  ...
  and (.durationMs | type == "number" and . >= 0)
  and ([.installations, .credentials, .permissions, .owners,
        .selectedPaused, .credentialsRevoked, .permissionsRemoved,
        .installationsDisconnected, .auditReceipts]
    | all(type == "number" and floor == . and . >= 0 and . <= 9007199254740991))
  and (
    if .reason == "executed" then
      .selectedPaused == .installations
      and .credentialsRevoked == .credentials
      and .permissionsRemoved == .permissions
      and .installationsDisconnected == .installations
      and .auditReceipts == 1
    elif .reason == "already-executed" then
      .selectedPaused == 0
      and .credentialsRevoked == 0
      and .permissionsRemoved == 0
      and .installationsDisconnected == 0
      and .auditReceipts == 0
    else false
    end
  )
...
unset plan_id expected_installations expected_credentials expected_permissions expected_owners
cleanup_emergency_files
trap - EXIT
```

The transaction selects Index, generation-fences/disables actionable Hermes installations, revokes every pending/active exact-audience credential, removes only `manage:negotiations` from locked Hermes permission rows, and writes one credential-free receipt. A count mismatch, plan drift, concurrency loss, or injected failure rolls the whole transaction back. An exact rerun is idempotent and reports `already-executed` with zero new mutations. Its transaction table and advisory locks end at commit; only the separately authorized admission fence prevents new authority afterward.

Never upload the plan ID, receipt ID, plan/receipt file, command output, raw logs, database URL, IDs, credentials, credential- or identity-derived hashes, or credential-derived data. The plan ID is operational confirmation material, not release evidence. The approved immutable public image digest is permitted only inside the sanitized compatibility report described under Evidence handling.
...
Required output is exactly three decimal zero counts. Also require the runtime dashboard to show Index coverage and no new dedicated pickup success. Confirm the admission fence is still active and its fixed denial probes still pass. Save only the three count names/values, fixed pass/fail, and approved timestamps; this is credential-free evidence.
...
4. Watch health/error/latency plus `hermes.server_error`, `hermes.index_fallback`, `hermes.pending_outbox`, and `hermes.advisory_lock_wait_ms`. Before considering restoration complete, re-verify the three zero counts while the fence remains held and repeat its fixed denial probes. Any nonzero count or admitted path stops rollback.
5. Keep the fence held. Prepare a forward fix. Re-enablement requires a new protected assurance run, new rollout approval, and fresh owner authorization; revoked credentials are never restored.

## Evidence handling

Permitted incident evidence is limited to fixed reasons, decimal counts, durations, timestamps, the approved immutable public image digest only inside the sanitized compatibility report, and pass/fail states. Existing compatibility/preflight reports and the aggregate assurance JSON are already sanitized schemas. Outside that report, do not attach the digest or any raw terminal/workflow logs, URL, ID, secret, request header/body, metadata payload, owner content, memory, consultation text, transcript prose, credential- or identity-derived hashes, or credential-derived material.
