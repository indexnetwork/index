...
| Privacy threshold precedent | Default-five cohort, hard-clamped to `[2,100]`, with sub-threshold suppression and aggregate-only diagnostics. Originally locked for frame-drift monitoring (recycled; restore from `061d0c51e`). | `061d0c51e:services/api/src/lib/frame-drift.config.ts` |
