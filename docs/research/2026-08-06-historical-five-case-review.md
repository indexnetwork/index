...
The recursive no-leak check now explicitly rejects report and audit key names, including report names, citations, claims, participant kinds, provenance, semantic negatives, cutoff/source-audit/review containers, and their relevant nested keys. It also rejects the specific reviewer IDs, review dates, and full review rationales from all five cases, while retaining the existing report-name, citation, and semantic-negative-reason checks. Generic values such as `approved` and `medium` are intentionally not treated as leak sentinels because legitimate serialized text could contain them independently.
