...
A follow-up audit of the exact merged `dev` checkpoint `b7a4d2a82af469a5ed1f035125d552fba8e83531` found that the readable base-seed serializer expectations were partly circular: transformed row content was checked against the same `HISTORICAL_MATRIX_CASES` corpus supplied to `baseSeedPayload`, so corpus and serialization drift could agree and pass together. The provider-free API serializer test now also hashes canonical test-only `JSON.stringify(payload)` output and compares the complete payload—all arrays, fields, and ordering—with the hand-captured SHA-256 digest `816fc4f9e7fa49f0c16ec10bdec0fb85029112eda7cf62bc54a565ab9b278c6d`. Updating that literal requires deliberate review of the full serialized payload; the readable field and reference assertions remain for diagnosis.

The recursive no-leak check now compares object keys by exact equality against the forbidden audit-key set. That set names the actual `cutoff` container (along with report names, citations, claims, participant kinds, provenance, semantic negatives, source-audit/review containers, and their relevant nested audit keys); it does not substring-match key sentinels such as `review`, `decision`, or `confidence` against model text. Separately, string values are checked for the specific report names, citation content, semantic-negative reasons, reviewer IDs, review dates, and full review rationales from all five cases. Generic values such as `approved` and `medium` are intentionally not treated as leak sentinels because legitimate serialized text could contain them independently.

Finally, H4's audit-only review rationale now says matrix integration was pending **at that review checkpoint**. This is a temporal clarification only: no model-facing H4 data, citation, decision, reviewer, date, or recognizability value changed. Session-local raw subagent evidence remains non-durable; this committed receipt and the corpus metadata are the durable audit record.
