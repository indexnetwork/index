...
Download only the `hermes-backend-production-assurance` aggregate artifact and the established previous-API compatibility JSON reports. The aggregate contains fixed gate names, result, approved thresholds, and the sanitized preflight report. Treat evidence as **credential-free**: never attach raw workflow logs, database URLs, owner/agent/installation/negotiation IDs, plan IDs, credential values or hashes, request headers, payloads, transcripts, or credential-derived material.
...
1. **Prepare.** From the approved client, start a fresh browser authorization and call `POST /api/agent-runtime/hermes/prepare` with exact UUID fields `{installationId,setupAttemptId}`. Complete PKCE consent/exchange and Keychain activation. Expect a pending generation before activation and an active exact generation afterward.
2. **Select Hermes.** Call `PUT /api/agent-runtime` with exactly `{runtime:"hermes",installationId,executorId,setupAttemptId}`. Read `GET /api/agent-runtime?installationId=...` and require selected Hermes with `health:"active"` and `indexCovering:false`.
...
7. **Reselect Hermes.** Call `PUT /api/agent-runtime` again with the same still-active exact generation. Require selected Hermes and a successful empty pickup (`pending:false`) or the next sanctioned turn.
8. **Disconnect.** Call `DELETE /api/agent-runtime/hermes/:installationId` with no body. Require selected Index, inactive installation, revoked dedicated credential, removed negotiation authority, and `indexCovering:true`. Reconnect must require fresh authorization rather than extending the old credential.
