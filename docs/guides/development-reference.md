...
**Tech Stack**: Bun runtime (Bun.serve), Drizzle ORM, PostgreSQL with pgvector, node-cron, LangChain/LangGraph
...
- `src/queues/` - Background-work orchestration: fire-and-forget async functions via `background()` (`src/lib/background.ts`), plus a couple of `node-cron` schedules. No queue, no Redis-backed job system — see `docs/design/architecture-overview.md` § Background Work.
...
2. **Services** import **adapters** for data access. Must not import other services -- use events, background work, or shared lib for cross-service orchestration.
