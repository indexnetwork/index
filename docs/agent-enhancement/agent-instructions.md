...
| Intent injection | Raw own intent orients both H2A and A2A | H2A retains canonical intent; A2A uses its private brief and intent identity. |
...
| Authority | Every commitment approval requires match scope | Use sufficient existing standing authority; any newly requested opportunity approval remains match-scoped. |
...
|---|---|---|
| Orientation | Raw own intent; optional `communicationReview` | Maintained H2A-owned `brief`; canonical intent ID |
...
| Principal evidence | Confirmed profile + full `principalConversation` | Retain scopes, references, dates and exact answer text |
| Other agreements | `acceptedCommitments` | `agreements`; observed agreement does not imply consent |
...
| Scope follows the saved question | A brief “yes” cannot approve another opportunity. |
