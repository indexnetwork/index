...
An `unresolved` delta is deliberately consumerless. It declares package-owned read/accessor-closure evidence plus a `noDirectProtocolConsumer` assertion. Generation enforces that assertion at a conservative module boundary rather than attempting unbounded JavaScript dataflow analysis: within the accessor module, any value reference outside the declared closure fails; outside it, any runtime named/default import of a closure export or any namespace import that can expose one fails immediately, regardless of whether later code calls it directly, aliases it, destructures it, or reaches it reflectively. Declaration-only barrel re-exports are permitted, but the first downstream runtime import from that barrel fails. Type-only imports and named imports proven to target other exports remain allowed. This makes newly wired behavior force reclassification without inventing a consumer or requiring a nonexistent behavior test.
...
- validates unresolved records by proving their package-owned read/accessor evidence, absence of same-module references outside the closure, and absence of runtime imports of the unresolved export through direct or barrel paths; a namespace import that could expose it fails closed;
