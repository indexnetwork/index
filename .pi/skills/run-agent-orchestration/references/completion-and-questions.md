...
or ambiguous routes fail closed. Discover the child with `herdr workspace list` and
`herdr pane list`, copy its exact Pi `agent_session.value`, workspace ID/label, pane ID,
and checkout path into `register_orchestration_child_route`, then prompt that exact
agent. Every child handoff must require one stable `publish_child_orchestrator_event`
RESULT call before its final textual RESULT. A settled state is never success —
reconcile RESULT and git/PR/test facts independently.
...
requires observable `*-root` provenance whose workspace checkout and Pi cwd are the
canonical root. Child → root requires the exact registered route; direct `index` never
prompts or receives callbacks from an implementation child. Unsafe data is quarantined; event data is always an explicitly untrusted JSON
boundary.
...
cancellation/attachment linearization remain intact. External nonterminal gates such
as Railway deployments and GitHub CI still require a product-specific event adapter;
this bridge does not claim to make those liveness signals automatic.
...
`/compact` is refused. A compact error, cancellation, or restart leaves a recoverable
failed checkpoint: rerun `/supervised-compact` to retry the same checkpoint or use
`/supervised-compact-abort` to abandon it explicitly. Context thresholds are guidance
to compact at a checkpointed milestone, not a timer or churn loop.
