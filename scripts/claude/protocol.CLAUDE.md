...
`skills/` holds the source templates for the SKILL.md files shipped in `packages/claude-plugin/` and `packages/hermes-plugin/`. **Edit the templates here**, then run `scripts/build-skills.ts` from the monorepo root to regenerate the output files. Never edit the generated SKILL.md files directly.
