...
IND-430 adds disabled-by-default, measurement-only daily monitoring through `FrameDriftQueue` → `FrameDriftMonitoringService` → `FrameDriftDatabaseAdapter`. In one repeatable-read transaction it records per-network premise/intent/user-context pgvector centroids and bounded canonical cross-network opportunity yield in `frame_centroid_snapshots` and `cross_network_yield_snapshots`. It has no API/UI and must never mutate embeddings, prompts, vocabulary, assignments, opportunities, or networks. BullMQ's stable scheduler is UTC and omitted from Bull Board; disabled startup removes any prior scheduler without creating a worker. See `docs/design/frame-drift-monitoring.md` for attribution, denominator, logging, bounds, and limitations.
