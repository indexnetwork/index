...
IND-506 adds a second ordinary intent-refinement moment after authoritative discovery success. Both `FromIntentQueue` and exact-intent `DiscoveryRunQueue` call one failure-isolated completion hook, which reuses the existing Questioner worker and master flag. `IntentRecoveryRefinementService` requires an exact active owned intent and suppresses generation when any exact-`detection.triggeredBy` opportunity passes canonical `canUserSeeOpportunity` plus `isActionableForViewer` (only role-eligible `latent`/`pending`). Generation receives payload/summary, stored global owner context, and at most a bounded aggregate count of rejected negotiations that pass bilateral participant, capture-time fingerprint, completed-task, task/actor network, and single no-opportunity-artifact validation; no IDs, identities, profiles, networks, turns/transcripts, reasons, presenter/evaluator text, or event context crosses the generation/persistence boundary. Rows remain public `mode='intent'` questions while private `purpose='recovery'` metadata is REST-stripped. Advisory/row locks and migration `0105` enforce one recipient+intent+material-fingerprint row across all statuses/expiry and recheck lifecycle/fingerprint/exact-trigger actionability immediately before insert. Material edits and stale answers system-void recovery rows without reaction; valid answers reuse canonical intent refinement/HyDE/rediscovery. Recovery and pool questions have independent cadence/budgets and may coexist.
