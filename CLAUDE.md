...
| `packages/a2a`, `packages/agent` | `@indexnetwork/a2a` — Agent2Agent negotiation client/server with an LLM negotiator; `@indexnetwork/agent` — a host-run personal agent over it (pins a2a exactly, so build a2a first). Both subtree-mirrored, published to npm, under the same pinning rule. |
