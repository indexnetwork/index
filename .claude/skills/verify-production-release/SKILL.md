...
description: "Pre- and post-merge safety checks for a dev→main release: a stale root bun.lock failing the prod build under --frozen-lockfile, a destructive Drizzle migration whose operational backfill never ran, dev→prod feature-flag drift, and removed queue workers orphaning prod Redis jobs. Use when cutting, merging, or auditing a release PR, or before merging any PR carrying a destructive migration."

---

# verify-production-release

Two production-release foot-guns observed shipping the `user_profiles` removal epic. Current CI catches lockfile drift, but Railway watched-path behavior and destructive-migration data readiness still deserve explicit release checks. Use this when promoting `dev`→`main` (pairs with `open-release-pr` for cutting the PR).
...
by rehearsing on the Neon dev branch — see `services/api/src/cli/AGENTS.md`, where
exactly that check exposed a write path that had never functioned since it shipped.

## See also
- [Feature flags](../../../docs/guides/feature-flags.md) — the four flag surfaces and the ship-dark→flip order behind check 3.
- `services/api/src/cli/AGENTS.md` — when a shipped backfill reports attempts but zero updates.
- [Development Reference → Git Workflow](../../../docs/guides/development-reference.md#git-workflow) — merge and post-merge deploy verification.
